...
class ResNet_Self_Attn(nn.Module):
    def __init__(self, depth, out_features=125):
        super(ResNet_Self_Attn, self).__init__()
        if depth == 18:
        ...
        self.avgpool = nn.AvgPool2d(7, stride=1)
        in_features_num = self.model.fc.weight.shape[1]
        self.fc = nn.Linear(in_features=in_features_num, out_features=out_features)
        self.attn = Self_Attn(in_features_num, 'relu')
